...
The Audio tab has master/effects volume, mute and a test sound. The Graphics tab contains fullscreen, antialiasing, the performance overlay and window resolution. Resolution choices fit the current screen with room for borders and the taskbar. Selecting one switches to windowed mode; fullscreen uses the desktop resolution. You can also resize the window manually. Mouse steering uses unscaled screen motion so viewport scaling does not change sensitivity.
